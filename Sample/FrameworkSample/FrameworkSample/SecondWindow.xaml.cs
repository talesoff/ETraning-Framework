using ETE.Engine;
using FrameworkSample.ViewModel;
using System;
...
namespace FrameworkSample
{
    ...
    public partial class SecondWindow : Window
    {
        ...
        private void SecondWindow_Loaded(object sender, RoutedEventArgs e)
        {
            ...
            HierarchyGroup.IsEnabled = false;
        }
            ...
            simulationWorker = new Thread(() =>
                {
                    try
                    {
                        ...
                        while (true == isRun)
                        {
                            ...
                            {
                                Action act = null;
                                if (true == actionQueue.TryDequeue(out act))
                                {
                                    act();
                                }
                            }
        ...
        {
            ETRViewModel.Instance.Stop();
            ETRViewModel.Instance.Reset();
        }

        #region Hierarchy
        private void AddTrianbleButton_Click(object sender, RoutedEventArgs e)
        {
            // 비동기 작업 추가
            actionQueue.Enqueue(new Action(() =>
            {
                ETRViewModel.Instance.AddObject();
            }));
        }

        private void RemoveTrianbleButton_Click(object sender, RoutedEventArgs e)
        {
            // 비동기 작업 추가
            actionQueue.Enqueue(new Action(() =>
            {
                ETRViewModel.Instance.RemoveObject();
            }));
        }
        #endregion
